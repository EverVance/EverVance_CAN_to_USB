...
uint32_t ProgramPage(uint32_t adr, uint32_t sz, uint8_t *buf) {
    ...
    while (sz) {
        chunk = FLASH_PAGE_SIZE - (adr & (FLASH_PAGE_SIZE - 1U));
        if (chunk > sz) {
            chunk = sz;
        }

    	status = flexspi_nor_flash_page_program(EXAMPLE_FLEXSPI, adr, chunk, (const uint32_t *)buf);

    	if (status) {
    		return (status);
    	}

    	sz = sz - chunk;		// Decrease remaining size by bytes actually programmed
    	adr = adr + chunk;	// Update flash address by bytes actually programmed
    	buf = buf + chunk;	// Next source chunk
