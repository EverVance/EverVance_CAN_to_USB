...
#define FLASH_4READ_DUMMY_CYCLES 0x06
#define FLASH_STATUS_QE_ENABLE   0x40
    ...
    .memConfig =
        {
            ...
            /*
             * MX25L3233F 的 QE 默认关闭，4READ(0xEB) 前必须先把状态寄存器 bit6 置 1。
             * 这里使用 ROM 支持的 QuadEnable 设备模式命令，在启动时通过 WRSR(0x01) 写入 0x40。
             */
            ...
            .deviceModeArg = FLASH_STATUS_QE_ENABLE,
            .lookupTable =
                {
                    // Read LUTs
                    [0] = FLEXSPI_LUT_SEQ(CMD_SDR, FLEXSPI_1PAD, 0xEB, RADDR_SDR, FLEXSPI_4PAD, 0x18),
                    [1] = FLEXSPI_LUT_SEQ(DUMMY_SDR, FLEXSPI_4PAD, FLASH_4READ_DUMMY_CYCLES, READ_SDR, FLEXSPI_4PAD, 0x04),
                    ...
                    // Quad Enable via WRSR(Status Register bit6 = QE)
                    [4 * 4 + 0] = FLEXSPI_LUT_SEQ(CMD_SDR, FLEXSPI_1PAD, 0x01, WRITE_SDR, FLEXSPI_1PAD, 0x01),
