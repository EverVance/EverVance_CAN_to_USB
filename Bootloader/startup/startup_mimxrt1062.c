...
#define APP_IMAGE_BASE_ADDR   0x60020000U
#define APP_VECTOR_TABLE_ADDR 0x60022000U

static int AppVectorLooksValid(uint32_t appStack, uint32_t appReset)
{
    const uint32_t isThumbAddress = (appReset & 1U);
    const uint32_t stackInDtcm = ((appStack & 0xFFF00000U) == 0x20000000U);
    const uint32_t stackInItcm = ((appStack & 0xFFF00000U) == 0x00000000U);
    const uint32_t resetInAppFlash =
        ((appReset & 0xFFFF0000U) >= APP_IMAGE_BASE_ADDR) &&
        ((appReset & 0xFFFF0000U) < 0x60400000U);

    return ((stackInDtcm || stackInItcm) && isThumbAddress && resetInAppFlash);
}

void JumpToApp(void)
{
    typedef void (*app_entry_t)(void);
    uint32_t appStack = *(uint32_t *)APP_VECTOR_TABLE_ADDR;
    uint32_t appReset = *(uint32_t *)(APP_VECTOR_TABLE_ADDR + 4U);
    uint32_t i;

    if (!AppVectorLooksValid(appStack, appReset))
    {
        return;
    }
