...
- `0x00`：配置已生效
- `0x01`：参数非法，设备拒绝
- `0x02`：配置已缓存，且已应用使能/终端；位时序待底层驱动接入
- `0x03`：设备内部错误
...
- `0x01 SetChannelConfig`
  - 请求：`16` 字节配置载荷
  - 响应：回显“设备当前生效配置”，载荷同 `SetChannelConfig`
- `0x02 GetChannelConfig`
  - 请求：无载荷，`Channel=目标通道`
  - 响应：`16` 字节当前配置载荷
- `0x03 GetDeviceInfo`
...
- 阶段 4 `通道配置`：协议已支持 `采样点/终端电阻/通道开关` 下发与回执；其中终端电阻与通道使能已落地到 GPIO/收发器控制，动态位时序仍处于缓存阶段。
- 当前主机侧有两条调试入口：
